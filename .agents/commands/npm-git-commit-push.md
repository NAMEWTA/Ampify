...
将分支名保存为 `<branch>`，后续标签格式为 `v<版本号>(<branch>)`。
例：分支 `main` → `v2.4.0(main)`，分支 `pure-ts` → `v2.2.1(pure-ts)`。
...
**注意**：最近标签可能带有分支后缀（如 `v1.5.0(main)`），提取版本号时需去除 `(...)` 部分，仅以纯版本号参与计算。

**版本计算规则**：在最新标签的 minor 版本上 +1，patch 归零。
- 例：`v1.5.0(main)` → `v1.6.0`
- 例：`v2.3.1(pure-ts)` → `v2.4.0`
- 例：`v0.0.0`（无标签）→ `v0.1.0`
...
标签格式为 `v<新版本号>(<branch>)`，其中 `<branch>` 为步骤 0 获取的分支名。

```bash
git tag "v<新版本号>(<branch>)"
git push <remote-name> "v<新版本号>(<branch>)"
```

例：分支 `main`，版本 `2.4.0` → 标签 `v2.4.0(main)`
