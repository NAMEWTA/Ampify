...
## 项目概述
Ampify 是一个 VS Code 扩展，用于快速复制“文件路径 + 行号”，以便在报告和代码评审中引用。扩展在激活后注册两个命令，分别复制相对路径与绝对路径，并将结果包裹在反引号中写入剪贴板。
...
## 项目结构
- [src/](src/)：扩展源代码
  - [src/extension.ts](src/extension.ts)：扩展入口与主要逻辑
